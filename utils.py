...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        ######################################################################################
        data = torch.load(os.path.join(data_path, 'tiny_imagenet.npz'), map_location='cpu')
...
'''
def combine_datasets(dataset1, dataset2, alpha=0.5, seed=21):
    if seed is not None:
        random.seed(seed)
    ...
    for img1, label1 in dataset1:
        label1 = label1.item()

        if label1 in label_to_indices:
            selected_idx = random.choice(label_to_indices[label1])
            img2, _ = dataset2[selected_idx]

            # print(f"Label1: {label1}, Label2: {_.item()}")

            # Combinar as imagens com o peso alpha para o conjunto "secundario"
            combined_img = (1 - alpha) * img1 + alpha * img2
            combined_images.append(combined_img)
            combined_labels.append(label1)
        else:
            raise ValueError(f"Label {label1} não encontrada no segundo dataset.")

    combined_images = torch.stack(combined_images)
    combined_labels = torch.tensor(combined_labels)

    return TensorDataset(combined_images, combined_labels)
'''
...
def combine_datasets_mixup(dataset1, dataset2, alpha):

    images1, labels1 = dataset1.images.cpu().numpy(), dataset1.labels.cpu().numpy()
    images2, labels2 = dataset2.images.cpu().numpy(), dataset2.labels.cpu().numpy()
    
    random_indices = np.random.choice(len(dataset2), size=len(dataset1))
    
    selected_images2 = images2[random_indices]
    selected_labels2 = labels2[random_indices]
    
    combined_images = (1 - alpha) * images1 + alpha * selected_images2
    combined_labels = (1 - alpha) * labels1 + alpha * selected_labels2
    
    combined_images_tensor = torch.tensor(combined_images).to('cuda')
    combined_labels_tensor = torch.tensor(combined_labels).to('cuda') # dtype=torch.float32 ???
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test), flush=True)

    return net, acc_train, acc_test


def evaluate_synset_ours(it_eval, net, images_train, labels_train, testloader, args, images_dataset, labels_dataset):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)

    ###
    images_dataset = images_dataset.to(args.device)
    labels_dataset = labels_dataset.to(args.device)
    dst_original = TensorDataset(images_dataset, labels_dataset) # dataset2
    ###
    ...
    dst_train = TensorDataset(images_train, labels_train) # dataset1
    # trainloader = torch.utils.data.DataLoader(dst_train, batch_size=args.batch_train, shuffle=True, num_workers=0)
    ...
    for ep in range(Epoch+1):
        if args.alpha == -1:
            ...
            alpha = args.alpha
        dst_combined = combine_datasets(dst_train, dst_original, alpha)
        trainloader = torch.utils.data.DataLoader(dst_combined, batch_size=args.batch_train, shuffle=True, num_workers=0)
    ...
    time_train = time.time() - start
    loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False)
    print('%s Evaluate_Ours%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test), flush=True)
